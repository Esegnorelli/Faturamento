...
def display_top_performers(df: pd.DataFrame, periodo_ini: str, periodo_fim: str) -> None:
    ...
    if not rank_df.empty:
        # Exibe o top 3 de lojas por número de pedidos (ranking tradicional)
        cols = st.columns(3)
        ...
        # Computa os melhores para cada métrica (faturamento, pedidos, ticket)
        agg_data = (
            df.loc[rank_mask]
            .groupby("loja", as_index=False)
            .agg(faturamento=("faturamento", "sum"), pedidos=("pedidos", "sum"))
        )
        # Evita divisões por zero ao calcular ticket
        agg_data["ticket"] = agg_data.apply(lambda r: safe_div(r["faturamento"], r["pedidos"]), axis=1)
