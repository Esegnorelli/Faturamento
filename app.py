...
st.sidebar.write("Períodos (marque os meses):")
all_p = st.sidebar.checkbox("Selecionar todos os meses", value=True, key="all_p")
sel_periodos = periodos if all_p else [p for p in periodos if st.sidebar.checkbox(p, value=False, key=f"p_{p}")]
if not sel_periodos:
    sel_periodos = periodos  # fallback
...
# Aplica filtros
mask = df["periodo"].isin(sel_periodos) & df["loja"].isin(sel_lojas)
...
st.write(
    f"Período marcado: {sel_periodos[0] if sel_periodos else '-'} até {sel_periodos[-1] if sel_periodos else '-'}  "
    f"|  Lojas selecionadas: {len(sel_lojas)}"
)

m1, m2, m3, m4 = st.columns(4)
with m1:
    st.metric(label="Faturamento no período", value=fmt_brl(k["period_sum"]["fat"]))
with m2:
    st.metric(label="Pedidos no período", value=fmt_int(k["period_sum"]["ped"]))
with m3:
    st.metric(
...
with col_a:
    if not serie_all.empty:
        ...
        fig_var = go.Figure()
        fig_var.add_trace(go.Scatter(x=serie_all["data"], y=100*serie_all["mom"], name="MoM %", mode="lines+markers"))
        fig_var.add_trace(go.Scatter(x=serie_all["data"], y=100*serie_all["yoy"], name="YoY %", mode="lines+markers"))
        fig_var.update_layout(height=330, yaxis_title="Variação (%)")
...
with col_c:
    if k["current"] is not None:
        ...
        if not tbl.empty:
            ...
            top_up = tbl.head(10)
            fig_up = px.bar(top_up, x="loja", y="mom_pct")
            fig_up.update_layout(height=330, yaxis_title="MoM (%)")
...
if k["current"] is not None:
    ...
    if not part.empty:
        ...
        fig_part.update_layout(height=380, xaxis_title="Participação (%)")
