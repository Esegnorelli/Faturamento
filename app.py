...
def detect_outliers(series: pd.Series, method: str = "zscore") -> pd.Series:
    ...
    if method == "zscore":
        # dropna to compute zscore correctly, then align with original index
        non_na = series.dropna()
        z_scores = np.abs(zscore(non_na))
        outliers = pd.Series(False, index=series.index)
        outliers.loc[non_na.index[z_scores > BENCHMARK_TARGETS["outlier_zscore"]]] = True
        return outliers
    elif method == "iqr":
...
class IntelligentKPI:
    ...
    def __init__(self, name: str, value: float, comparison_value: Optional[float] = None,
                 format_func: Optional[Callable[[float], str]] = None, icon: str = "📊"):
        self.name = name
        self.value = value
    ...
    def get_explanation(self) -> str:
        ...
        explanations = {
            ...
            "Consistência": "Indica quão consistente é a série ao longo do tempo.",
            "Momentum": "Aceleração recente nas vendas em relação ao passado.",
        }
        return explanations.get(self.name, "KPI de análise de performance.")
    
    def render(self) -> None:
        """Renderiza o KPI com explicação interativa."""
        delta_text = f"{self.delta*100:+.1f}%" if self.delta is not None else None
        performance_class, icon = self.performance_class
        value_formatted = self.format_func(self.value) if self.format_func else f"{self.value}"
...
def compute_advanced_kpis(df_filtered: pd.DataFrame, df_historical: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(serie_atual) > 1:
        first_val = serie_atual.iloc[0]
        last_val = serie_atual.iloc[-1]
        periods = len(serie_atual) - 1
        growth_rate = ((last_val / max(1, first_val)) ** (1/periods)) - 1
    ...
    if total_pedidos > 0 and df_historical["pedidos"].sum() > 0:
        ticket_historico = df_historical["faturamento"].sum() / max(1, df_historical["pedidos"].sum())
        eficiencia = (ticket_medio / max(1e-9, ticket_historico)) - 1
    ...
    if len(serie_atual) >= 6:
        ...
        momentum = (recent - previous) / max(1e-9, previous)
...
def render_enhanced_top_performers(df: pd.DataFrame, periodo_ini: str, periodo_fim: str) -> None:
    ...
    for idx, (_, row) in enumerate(top_5.iterrows()):
        ...
        fat_norm = (row["faturamento_sum"] / top_5["faturamento_sum"].max()) * 100 if top_5["faturamento_sum"].max() > 0 else 0
        ped_norm = (row["pedidos_sum"] / top_5["pedidos_sum"].max()) * 100 if top_5["pedidos_sum"].max() > 0 else 0
        ticket_norm = (row["ticket_mean"] / top_5["ticket_mean"].max()) * 100 if top_5["ticket_mean"].max() > 0 else 0
        consist_norm = 100 - ((row.get("faturamento_std", 0) / row["faturamento_mean"]) * 50) if row.get("faturamento_mean", 0) > 0 else 50
...
def render_clustering_analysis(df: pd.DataFrame) -> None:
    ...
    n_clusters = min(4, max(2, len(features_data) // 2 + 1))
...
def render_forecasting_module(kpis_data: Dict[str, Any], periods: int = 6) -> None:
    ...
    try:
        ...
        z_score_map = {80: 1.28, 90: 1.64, 95: 1.96}
        z_score_val = z_score_map.get(confidence_level, 1.64)
        confidence_interval = z_score_val * forecast_std
        ...
        with forecast_col3:
            ...
            total_atual_series = ts_data.tail(forecast_periods)
            total_atual = total_atual_series.sum() if not total_atual_series.empty else 0
...
def render_anomaly_detection(kpis_data: Dict[str, Any]) -> None:
    ...
    z_scores_series = np.abs(zscore(serie))
    anomalias_zscore = z_scores_series > BENCHMARK_TARGETS["outlier_zscore"]
...
def render_business_insights_engine(kpis_data: Dict[str, Any], df: pd.DataFrame) -> None:
    ...
    if "efficiency" in intelligent_kpis:
        ...
        elif eff_class == "atencao":
            insights.append({
                "type": "warning",
                "title": "Eficiência Reduzida", 
                "description": f"Performance {eff_kpi.value*100:+.1f}% abaixo do histórico.",
                "action": "Revisar processos operacionais e treinamento.",
                "priority": "media"
            })
    ...
    with st.expander("📊 Explicação dos KPIs Principais"):
        ...
        with tab3:
            ...
            **Z-Score (Detecção de Anomalias)**: Identifica períodos atípicos
            ...
            - **Uso**: Investigir causas de variações extremas
...
def render_advanced_evolution_charts(kpis_data: Dict[str, Any]) -> None:
    ...
    if len(df_evolution) > 3:
        x_vals = np.arange(len(df_evolution))
        coeffs = np.polyfit(x_vals, df_evolution["faturamento"], 1)
        df_evolution["tendencia"] = np.poly1d(coeffs)(x_vals)
    ...
    if HAS_STATSMODELS and len(serie) > 10:
        try:
            ...
            autocorr_vals = [serie.autocorr(lag=i) for i in range(1, lags + 1)]
            fig_evolution.add_trace(
                go.Bar(x=list(range(1, lags + 1)), y=autocorr_vals,
